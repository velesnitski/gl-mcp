...
pub(crate) fn render_benchmark_md(b: &Benchmark) -> Vec<String> {
    ...
    out.push(
        "_Benchmarks & practices: [Claude Code best practices](https://www.anthropic.com/engineering/claude-code-best-practices) \u{b7} [AGENTS.md](https://agents.md/) \u{b7} [12-Factor Agents](https://github.com/humanlayer/12-factor-agents) \u{b7} [Pragmatic Engineer: AI tooling survey](https://newsletter.pragmaticengineer.com/p/ai-tooling-2026) \u{b7} [DORA 2025: State of AI-assisted development](https://dora.dev/dora-report-2025/)._"
            .to_string(),
    );
    out.push(String::new());
    out
...
mod tests {
    ...
    fn benchmark_rendered_by_both_paths() {
        ...
        for r in [md.as_str(), html.as_str()] {
            ...
            assert!(r.contains("humanlayer/12-factor-agents"));
            assert!(!r.contains("llmstxt.org"));
            // Redundant docs link dropped (best-practices covers it).
            assert!(!r.contains("docs.claude.com"));
        }
    }
